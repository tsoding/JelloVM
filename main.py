...
"JVM Byte-code Parser in Python"
...
def parse_u1(f: io.BufferedReader) -> int: return int.from_bytes(f.read(1), 'big')
def parse_u2(f: io.BufferedReader) -> int: return int.from_bytes(f.read(2), 'big')
def parse_u4(f: io.BufferedReader) -> int: return int.from_bytes(f.read(4), 'big')

def parse_attributes(f: io.BufferedReader, count: int) -> list[dict[str, int | bytes]]:
    "Parse attributes from f"
    attributes = []
...
def cli_run() -> None:
    ...
    clazz = parse_class_file(filename)
    pp.pprint(clazz)

if __name__ == '__main__':# Do not run if imported as module
    cli_run()
